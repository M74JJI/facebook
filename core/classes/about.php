...
class About extends User{
    ...
    public function overview($section,$userid,$profileid,$text,$icon){
        ...
        echo (($userid != $profileid)
         ? '<span class="about_show">'.$userInfos->$section.'</span><br>'
        : (($userInfos->$section == '') 
        ? '<div class="add-'.$section.' about_flex" data-userid="'.$userid.'" data-profileid="'.$profileid.'"><div class="add_about_icon"></div><div>'.$text.'</div></div>'
         : '<div class="add-'.$section.' about-flex" data-userid="'.$userid.'" data-profileid="'.$profileid.'"><img src="'.$icon.'" alt=""><span class="about_show">'.$userInfos->$section.'</span></div><br>'));
    ...
    public function getAboutPhone($section,$userid,$profileid,$text,$icon){
        $userInfos=$this->getUserInfo($profileid);
        echo (($userid != $profileid)
         ? '<span class="about_show">'.$userInfos->$section.'</span><br>'
        : (($userInfos->$section == '') 
        ? '<div class="add-'.$section.' about_flex" data-userid="'.$userid.'" data-profileid="'.$profileid.'"><div class="add_about_icon"></div><div>'.$text.'</div></div>'
         ...
         "><span class="about_show" style="font-size:15px">'.$userInfos->$section.'</span><span style="font-size:13px">Mobile</span></div>
